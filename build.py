...
def main():
    ...
    if "-v" in parameters:
        ...
        os.system(f"nuitka --onefile --follow-imports --enable-plugin=pyqt6 -o aviso-bot-debug --disable-console --output-dir=build --windows-icon-from-ico=./windows/icon.ico --include-data-files=./windows/icon.ico=windows/icon.ico --include-data-files=./res/version.json=res/version.json --deployment --company-name=Lil4ipsik --product-name=\"Aviso Bot\" --product-version={version(True)} main.py")
