...
def main():
    ...
    if "-v" in parameters:
        ...
        os.system(f"python -m nuitka --onefile --follow-imports --enable-plugin=pyqt6 -o aviso-bot --output-dir=build --linux-onefile-icon=./windows/icon.ico --include-data-files=./linux/icon.png=linux/icon.png --include-data-files=./res/version.json=res/version.json --deployment --company-name=Lil4ipsik --product-name=\"Aviso Bot\" --product-version={version(True)} main.py")
    else:
